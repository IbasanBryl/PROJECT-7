...
class AttendanceSystem:
    #Main class for Student Attendance Monitoring System!
    ...
    def _load_data(self):
        #Load data from JSON files, or initialize if files don't exist.
        # Load teachers
        if os.path.exists(self.teachers_file):
            with open(self.teachers_file, 'r') as f:
                self.teachers = json.load(f)
        else:
            self.teachers = {"admin": "admin123", "teacher1": "pass123"}
            self._save_teachers()
        ...
        if os.path.exists(self.students_file):
            ...
            sample_students = [
                {"id": "S001", "name": "Bryl", "grade": "BSIT-02"},
                {"id": "S002", "name": "Philip", "grade": "BSIT-02"},
                {"id": "S003", "name": "Elijah", "grade": "BSIT-02"},
                {"id": "S004", "name": "Kurt", "grade": "BSIT-02"},
                {"id": "S005", "name": "Nico", "grade": "BSIT-02"}
            ]
    ...
    def _save_teachers(self):
        #Save teachers data to a JSON file.
        with open(self.teachers_file, 'w') as f:
            json.dump(self.teachers, f, indent=4)
    
    def add_student(self, student_id: str, name: str, grade: str) -> tuple[bool, str]:
        #Add a new student
        try:
            # Validation
            ...
            if not re.match(r'^S\d{3,}$', student_id):
                return False, "Student ID must start with 'S' followed by numbers (e.g., S001)"
    ...
    def mark_attendance(self, student_id: str, status: str) -> tuple[bool, str]:
        ...
        try:
            ...
            record = {
                ...
                "section": self.students[student_id]["grade"],
                "status": status,
                "date": datetime.now().strftime("%Y-%m-%d"),
                "time": datetime.now().strftime("%H:%M:%S"),
                "marked_by": self.current_teacher
            }
    ...
    def get_student_attendance_history(self, student_id: str) -> List[Dict]:
        ...
        try:
            return [record for record in self.attendance_records 
    ...
    def get_statistics(self) -> Dict:
        ...
        try:
            ...
            today_records = self.get_daily_report(today)
...
class AttendanceGUI:
    ...
    def show_view_students(self):
        ...
        total_label = tk.Label(
            self.content_area,
            text=f"Total Students: {len(self.system.students)}",
            font=("Arial", 12, "bold"),
            bg="white"
        )
